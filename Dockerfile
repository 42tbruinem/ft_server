...
COPY	/srcs/index_on.conf /etc/nginx/conf.d/index_on.conf
...
RUN		ls -s /etc/nginx/sites-available/server.conf /etc/nginx/sites-enabled/server.conf
...
CMD service nginx restart && service mysql start && mysql_secure_installation && service apache2 start && service php7.3-fpm start && bash
